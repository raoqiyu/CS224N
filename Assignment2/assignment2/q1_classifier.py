...
class SoftmaxModel(Model):
    ...
    def add_prediction_op(self):
        ...
        """
        ### YOUR CODE HERE
        self.W_out = tf.Variable(tf.random_uniform([self.config.n_features,
                        self.config.n_classes]))
        self.b_out = tf.Variable(tf.random_uniform([self.config.n_classes]))

        pred = softmax(tf.matmul(self.input_placeholder,self.W_out) + self.b_out)
        ### END YOUR CODE
        return pred
