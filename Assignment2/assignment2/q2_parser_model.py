...
class ParserModel(Model):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE
        self.embedding_tensor = tf.Variable(self.pretrained_embeddings,trainable=False)
        embeddings = tf.nn.embedding_lookup(self.embedding_tensor,self.input_placeholder)
    ...
    def add_prediction_op(self):
        ...
        Returns:
            pred: tf.Tensor ofdp shape (batch_size, n_classes)
        """
        x = self.add_embedding()
        ### YOUR CODE HERE
        Weight_initial = xavier_weight_init()
        with tf.variable_scope('prediction'):
            W = Weight_initial([self.config.n_features*self.config.embed_size, self.config.hidden_size])
            b1 = tf.Variable(tf.zeros([self.config.hidden_size]))

            U = Weight_initial([self.config.hidden_size, self.config.n_classes])
