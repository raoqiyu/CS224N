...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    ### YOUR CODE HERE (5-20 lines)
        ...
        for idx in range(window_size, len(sentence)-window_size):
            #print idx
            x_ = sentence[idx-window_size:idx+window_size+1]
...
class WindowModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        with tf.variable_scope("prediction"):
            W = tf.Variable(Weight_initialize((self.config.n_window_features*\
                                                        self.config.embed_size,
                                                        self.config.hidden_size)))
            b1 = tf.Variable(tf.zeros(self.config.hidden_size))
            
            U = tf.Variable(Weight_initialize((self.config.hidden_size,
