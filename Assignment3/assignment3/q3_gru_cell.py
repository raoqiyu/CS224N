...
class GRUCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            self.W_r = tf.get_variable(name='W_r', shape=(self._state_size,self._state_size),dtype=tf.float32,
                                    initializer=tf.contrib.layers.xavier_initializer())
            self.U_r = tf.get_variable(name='U_r', shape=(self.input_size,self._state_size),dtype=tf.float32,
                                    initializer=tf.contrib.layers.xavier_initializer())
            self.b_r = tf.get_variable(name='b_r',shape=(self._state_size))

            self.W_z = tf.get_variable(name='W_z', shape=(self._state_size,self._state_size),dtype=tf.float32,
                                    initializer=tf.contrib.layers.xavier_initializer())
            self.U_z = tf.get_variable(name='U_z', shape=(self.input_size,self._state_size),dtype=tf.float32,
                                    initializer=tf.contrib.layers.xavier_initializer())
            self.b_z = tf.get_variable(name='b_z',shape=(self._state_size))

            self.W_o = tf.get_variable(name='W_o', shape=(self._state_size,self._state_size),dtype=tf.float32,
                                    initializer=tf.contrib.layers.xavier_initializer())
            self.U_o = tf.get_variable(name='U_o', shape=(self.input_size,self._state_size),dtype=tf.float32,
                                    initializer=tf.contrib.layers.xavier_initializer())
            self.b_o = tf.get_variable(name='b_o',shape=(self._state_size))

            z_t = tf.nn.sigmoid(tf.matmul(inputs,self.U_z) + tf.matmul(state,self.W_z) + self.b_z)
            r_t = tf.nn.sigmoid(tf.matmul(inputs,self.U_r) + tf.matmul(state,self.W_r) + self.b_r)
            o_t = tf.nn.tanh(tf.matmul(inputs,self.U_o) + tf.matmul(r_t*state,self.W_o) + self.b_o)
