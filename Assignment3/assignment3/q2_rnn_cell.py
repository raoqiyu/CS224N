...
class RNNCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        Weight_initialize = tf.contrib.layers.xavier_initializer()
        with tf.variable_scope(scope):
            ### YOUR CODE HERE (~6-10 lines)
            self.W_x = tf.get_variable(name='W_x', shape=(self.input_size,self._state_size),dtype=tf.float32,
                                    initializer=tf.contrib.layers.xavier_initializer())
            self.W_h = tf.get_variable(name='W_h', shape=(self._state_size,self._state_size),
                                    dtype=tf.float32,initializer=tf.contrib.layers.xavier_initializer())
            self.b = tf.get_variable(name='b',shape=(self._state_size))

            # self.W_x =  tf.Variable(Weight_initialize((self.input_size,self._state_size)))
            # self.W_h =  tf.Variable(Weight_initialize((self._state_size,self._state_size)))
            # self.b = tf.Variable(tf.zeros(self._state_size))

        new_state = tf.nn.sigmoid(tf.matmul(inputs,self.W_x,name='Wx') + tf.matmul(state,self.W_h,name='Wb') + self.b)
            ### END YOUR CODE ###
