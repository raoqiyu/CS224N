...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        lab = labels[:max_length]
        this_len = len(sen)
        #print this_len, max_length,sentence
        #if this_len < max_length:
        sen.extend([zero_vector for _ in xrange(max_length-this_len)])
        lab.extend([zero_label for _ in xrange(max_length-this_len)])
        mask = [True for _ in xrange(this_len)] + [False for _ in xrange(max_length-this_len)]
        ret.append((sen,lab,mask))
...
class RNNModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        with tf.variable_scope("predictions"):
            self.U = tf.get_variable(name='U', shape=(self.config.hidden_size,self.config.n_classes),
                                    initializer=tf.contrib.layers.xavier_initializer())
            self.b_2 = tf.get_variable(name='b_2',shape=(self.config.n_classes))
        ...
        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ...
                o_t, h_t = cell(x[:,time_step,:], h_t,scope="RNN_cell")
                o_drop_t = tf.nn.dropout(o_t, dropout_rate)
                y_t = tf.matmul(o_drop_t,self.U) + self.b_2
                preds.append(y_t)
                ### END YOUR CODE
    ...
    def add_training_op(self, loss):
        ...
        Args:
            loss: Lo20ss tensor, from cross_entropy_loss.
        Returns:
